	...
	unsigned int actualOctree)
{
	TQueI que;
	initQueI(&que);
	addQueItemI(&que, actualOctree);
	int stack_id = 0;

	while (stack_id >= 0) {
		actualOctree = getQueItemI(&que);
		removeQueItemI(&que);
		stack_id--;

		float mMin, mMax;
		if (!boxIntersect(ray, octree[actualOctree].boxMin, octree[actualOctree].boxMax, &mMin, &mMax)) {
			continue;
		}
		...
		if (!octree[actualOctree].leaf) {
			for (int i = 0; i < 8; i++) {
				stack_id++;
				addQueItemI(&que, octree[actualOctree].children[i]);
			}
		}
		else {
			TSphere sphere;
	...
	__global TOctreeBox* octree, unsigned int octree_size, __global TObject* objects)
{
	...
	while (true) {
		...
		if (intersectIs_octree(&ray, &is, sp, cnt_sp, tr, cnt_tr, me, ra_me, cnt_ra_me, 
							   octree, octree_size, objects, 0)) { // Not tested
